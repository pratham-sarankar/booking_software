...
@php    
    ...
    SEOTools::setTitle(trans('Forgot Password') . ' - ' . $page->meta_title);
    SEOTools::setDescription($page->meta_description);

    SEOMeta::setTitle(trans('Forgot Password') . ' - ' . $page->meta_title);
    SEOMeta::setDescription($page->meta_description);
    SEOMeta::addMeta('article:section', 'Forgot Password' . ' - ' . $page->meta_description, 'property');
    SEOMeta::addKeyword([$page->meta_keywords]);

    OpenGraph::setTitle(trans('Forgot Password') . ' - ' . $page->meta_title);
    OpenGraph::setDescription($page->meta_description);
    OpenGraph::setUrl(URL::full());
    OpenGraph::addImage([asset($setting->site_logo), 'size' => 300]);

    JsonLd::setTitle(trans('Forgot Password') . ' - ' . $page->meta_title);
