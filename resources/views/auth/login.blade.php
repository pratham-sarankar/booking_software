...
@php
    ...
    SEOTools::setTitle(trans('Login') . ' - ' . $page->meta_title);
    SEOTools::setDescription($page->meta_description);

    SEOMeta::setTitle(trans('Login') . ' - ' . $page->meta_title);
    SEOMeta::setDescription($page->meta_description);
    SEOMeta::addMeta('article:section', 'Login' . ' - ' . $page->meta_description, 'property');
    SEOMeta::addKeyword([$page->meta_keywords]);

    OpenGraph::setTitle(trans('Login') . ' - ' . $page->meta_title);
    OpenGraph::setDescription($page->meta_description);
    OpenGraph::setUrl(URL::full());
    OpenGraph::addImage([asset($setting->site_logo), 'size' => 300]);

    JsonLd::setTitle(trans('Login') . ' - ' . $page->meta_title);
                    ...
                    <div class="p-4 py-16 flex flex-col justify-center h-full md:max-w-lg mx-auto">
                        <form method="POST" action="{{ route('login') }}">
                            ...
                            <label class="block mb-4">
                                ...
                                <input
                                    class="px-4 py-3.5 w-full text-gray-400 font-medium placeholder-gray-400 bg-white outline-none border border-gray-300 rounded-lg focus:border-none focus:ring focus:ring-{{ $config[11]->config_value }}-300"
                            ...
                            <label class="block mb-5">
                                ...
                                <input
                                    class="px-4 py-3.5 w-full text-gray-400 font-medium placeholder-gray-400 bg-white outline-none border border-gray-300 rounded-lg focus:border-none focus:ring focus:ring-{{ $config[11]->config_value }}-300"
                                    id="password" type="password" name="password" value="{{ old('password') }}" required autocomplete="current-password"
                                    placeholder="********">
                            </label>
                                ...
                                <div class="w-auto p-2">
                                    <div class="flex items-center">
                                        <input id="remember_me" type="checkbox"
                                            class="w-4 h-4 rounded border-gray-300 text-{{ $config[11]->config_value }}-600 shadow-sm focus:ring focus:ring-{{ $config[11]->config_value }}-500"
                                            name="remember" />
                                        <label for="remember_me"
                                            class="ml-2 text-sm text-gray-900 font-medium">{{ __('Remember Me') }}</label>
                                    </div>
                                </div>
                                @if (Route::has('password.request'))
                                    <div class="w-auto p-2">
                                        @if (Route::has('password.request'))
                                            <a class="text-sm text-{{ $config[11]->config_value }}-600 hover:text-{{ $config[11]->config_value }}-700 font-medium"
                            ...
                            <button
                                ...
                                type="submit" id="login-form">{{ __('Log in') }}</button>
                        </form>
                                ...
                                <div class="w-full p-2 ">
                                    <button onclick="window.location.href='/auth/google'"
                                        ...
                                        <img class="mr-3" src="{{ asset('home-assets/logos/brands/google.svg') }}"
                                            alt="Google logo">
                                        <span class="font-semibold leading-normal">{{ __('Sign in with Google') }}</span>
                                    </button>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="text-center text-sm text-gray-500 font-medium mt-4 ">
                            <p>{{ __('Don’t have an account?') }} <a href="/register?type=user"
