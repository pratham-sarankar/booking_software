            ...
            <div class="page-header d-print-none">
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <h2 class="page-title">
                            {{ __('Business Offline Transactions') }}
                        </h2>
                    </div>
                </div>
            </div>
                                    ...
                                    <tbody>
                                        @foreach ($transactions as $transaction)
                                            <tr>
                                                ...
                                                <td class="text-start">{{ $transaction->transaction_id }}
                                                </td>
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end" style="">
                                                            ...
                                                            @if ($transaction->transaction_status != 'completed')
                                                                <a class="dropdown-item" href="#"
                                                                    onclick="getOfflineTransaction('{{ $transaction->transaction_id }}'); return false;">{{ __('success') }}</a>
                                                            @endif
                                                            @if ($transaction->transaction_status != 'pending')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.business.offline.trans.status', ['id' => $transaction->transaction_id, 'status' => 'pending']) }}">{{ __('pending') }}</a>
                                                            @endif
                                                            @if ($transaction->transaction_status != 'failed')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.business.offline.trans.status', ['id' => $transaction->transaction_id, 'status' => 'failed']) }}">{{ __('failed') }}</a>
                                                            @endif
                                                        </div>
                                                    </span>
                                                </td>
...
@section('custom-js')
    ...
    <script>
        function getOfflineTransaction(parameter) {
            ...
            link.setAttribute("href", "/admin/offline-transaction-status/" + parameter + "/completed");
        }
    </script>
@endsection
@endsection
