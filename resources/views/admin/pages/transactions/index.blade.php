            ...
            <div class="page-header d-print-none">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto ms-auto d-print-none">
                        <a type="button" href="{{ route('admin.business.offline.transactions') }}" class="btn btn-primary">
                                    ...
                                    <tbody>
                                        @foreach ($transactions as $transaction)
                                            <tr>
                                                ...
                                                <td class="text-start">
                                                    {{ $transaction->transaction_total != 0.0 ? $transaction->transaction_id : '-' }}
                                                </td>
                                                <td class="text-start"><a
                                                        href="{{ route('admin.view.user', $transaction->userId) }}">{{ $transaction->name }}</a>
                                                </td>
                                                <td class="text-start">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end" style="">
                                                            @if ($transaction->transaction_total != 0.0 && $transaction->transaction_status == 'completed')
                                                                <a class="dropdown-item" target="_blank"
                                                                    href="{{ route('admin.view.invoice', ['id' => $transaction->transaction_id]) }}">{{ __('Invoice') }}</a>
                                                            @endif
                                                            @if ($transaction->transaction_status != 'pending')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.trans.status', ['id' => $transaction->transaction_id, 'status' => 'pending']) }}">{{ __('Pending') }}</a>
                                                            @endif
                                                            @if ($transaction->transaction_status != 'completed')
                                                                <a class="dropdown-item" href="#"
                                                                    onclick="getTransaction('{{ $transaction->transaction_id }}'); return false;">{{ __('Success') }}</a>
                                                            @endif
                                                            @if ($transaction->transaction_status != 'failed')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.trans.status', ['id' => $transaction->transaction_id, 'status' => 'failed']) }}">{{ __('Failed') }}</a>
                                                            @endif
                                                        </div>
                                                    </span>
                                                </td>
...
@section('custom-js')
    ...
    <script>
        function getTransaction(parameter) {
            ...
            link.setAttribute("href", "/admin/business-transaction-status/" + parameter + "/completed");
        }
    </script>
@endsection
@endsection
