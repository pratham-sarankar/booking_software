            ...
            <div class="page-header d-print-none">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto ms-auto d-print-none">
                        <a type="button" href="{{ route('admin.plans.add') }}" class="btn btn-primary">
                                ...
                                <table class="table table-vcenter card-table" id="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-start">{{ __('Plan Price') }}</th>
                                            <th class="text-start">{{ __('Plan Validity') }}</th>
                                    ...
                                    <tbody>
                                        @foreach ($plans as $plan)
                                            <tr>
                                                ...
                                                <td class="text-start">{{ __($plan->plan_name) }}</td>
                                                <td class="text-start">
                                                    @if ($plan->plan_price == 0)
                                                        {{ __('Free') }}
                                                    @else
                                                        {{ $currency->symbol }}{{ $plan->plan_price }}
                                                    @endif
                                                </td>
                                                <td class="text-start">
                                                    ...
                                                    @elseif ($plan->plan_validity == '31')
                                                        {{ __('Monthly') }}
                                                    @elseif ($plan->plan_validity == ('365' || '366'))
                                                        {{ __('Yearly') }}
                                                    @else
                                                ...
                                                <td class="text-center">
                                                    <span class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end" style="">
                                                            ...
                                                            @endif
                                                            <a class="dropdown-item" href="#"
                                                                onclick="deletePlan('{{ $plan->plan_id }}'); return false;">{{ __('Delete') }}</a>
                                                        </div>
                                                    </span>
                                                </td>
                                            </tr>
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary me-auto"
                        data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <a class="btn btn-danger" id="delete_plan_id">{{ __('Yes, proceed') }}</a>
                </div>
            </div>
        </div>
    </div>
...
@section('custom-js')
    ...
    <script>
        ...
        function getPlan(parameter) {
            ...
            link.setAttribute("href", "{{ route('admin.activation.plan') }}?plan_id=" + parameter);
        }

        // Delete plan
        function deletePlan(parameter) {
            "use strict";
            $("#delete-modal").modal("show");
            var link = document.getElementById("delete_plan_id");
            link.setAttribute("href", "{{ route('admin.delete.plan') }}?plan_id=" + parameter);
        }
    </script>
@endsection
@endsection
