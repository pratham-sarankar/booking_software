...
@section('content')
    <div class="page-wrapper h-screen">
        <div class="page-body">
            {{-- Edit --}}
            <div class="w-full">
                <form action="{{ route('user.update.password') }}" method="post" class="p-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        {{-- New Password --}}
                        <div>
                            <label class="block mb-2 font-bold text-lg" for="new_password">{{ __('New Password') }}</label>
                        ...
                        <div>
                            ...
                            <input type="password" name="confirm_password" placeholder="{{ __('Confirm Password') }}"
                                class="w-full p-4 text-sm placeholder-gray-500 border border-gray-300 focus:border-none focus:ring focus:ring-{{ $config[11]->config_value }}-200 rounded-md appearance-none outline-none"
                                required />
                        </div>
                    </div>
