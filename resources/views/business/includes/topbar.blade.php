            ...
            <div class="nav-item dropdown">
                <a href="#" class="nav-link d-flex lh-1 text-reset p-0" data-bs-toggle="dropdown"
                    ...
                    <span class="avatar avatar-md">
                        <img src="{{ Auth::user()->profile_image == null ? asset($setting->favicon) : asset(Auth::user()->profile_image) }}"
                            alt="{{ Auth::user()->name }}">
                    </span>
                    <div class="d-none d-xl-block ps-2">
                        <div>{{ __(Auth::user()->name) }}</div>
                        <div class="mt-1 small text-muted">{{ __('Business') }}</div>
                    </div>
                </a>
                <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                    <a href="{{ route('business.index.account') }}"
                        class="dropdown-item">{{ __('Profile & Account') }}</a>
                    {{-- Light / Dark Mode --}}
                    <a href="{{ route('business.change.theme', 'dark') }}" class="dropdown-item hide-theme-dark"
                        data-bs-placement="bottom">
                        {{ __('Enable dark mode') }}
                    </a>
                    <a href="{{ route('business.change.theme', 'light') }}" class="dropdown-item hide-theme-light"
                        data-bs-placement="bottom">
