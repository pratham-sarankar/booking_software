...
<section class="relative py-16 bg-white overflow-hidden">
    <img class="absolute top-0 left-0 h-full" src="{{ asset('home-assets/images/footers/gradient4.svg') }}"
        alt="" />
    <div class="relative z-10 container px-4 mx-auto">
        <div class="flex flex-wrap justify-between items-center -m-8">
            ...
            <div class="w-auto p-8">
                <ul class="flex flex-wrap items-center -m-5">                    
                    <li class="p-5">
                        <a class="text-gray-600 hover:text-gray-700 font-medium"
                            href="{{ route('web.blogs') }}">{{ __('Blogs') }}</a>
                    </li>
                    <li class="p-5">
                        <a class="text-gray-600 hover:text-gray-700 font-medium" href="/#faq">{{ __('FAQs') }}</a>
                    </li>
                    @if ($pages[8]->page_slug == 'privacy-policy' && $pages[8]->status == 1)
                    <li class="p-5">
                        <a class="text-gray-600 hover:text-gray-700 font-medium"
                            href="{{ route('web.privacy') }}">{{ __('Privacy Policy') }}</a>
                    </li>
                    @endif
                    @if ($pages[9]->page_slug == 'refund-policy' && $pages[9]->status == 1)
                    <li class="p-5">
                        <a class="text-gray-600 hover:text-gray-700 font-medium"
                            href="{{ route('web.refund') }}">{{ __('Refund Policy') }}</a>
                    </li>
                    @endif
                    @if ($pages[10]->page_slug == 'terms-and-conditions' && $pages[10]->status == 1)
                    <li class="p-5">
                        <a class="text-gray-600 hover:text-gray-700 font-medium"
