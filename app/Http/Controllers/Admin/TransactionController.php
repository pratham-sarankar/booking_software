...
class TransactionController extends Controller
{
    ...
    public function index()
    {
        ...
        $transactions = Transaction::where('payment_gateway_name', '!=', 'Offline')
            ->orderBy('id', 'desc')
            ->get();
        ...
        for ($i = 0; $i < count($transactions); $i++) {
            ...
            if ($user_details) {
                ...
                $transactions[$i]['name'] = 'Business';
                $transactions[$i]['userId'] = '#';
            }
        }

        // View page
        return view('admin.pages.transactions.index', compact('transactions', 'settings', 'currency'));
    }
    ...
    public function offlineTransactionStatus($id, $status)
    {
        ...
        if ($status == "completed") {
            ...
            $transaction_details = Transaction::where('transaction_id', $id)->where('status', 1)->first();
            $user_details = User::where('user_id', $transaction_details->user_id)->first();
            ...
            if ($user_plan_details['plan_validity'] == "") {
                ...
                User::where('user_id', $transaction_details->user_id)->update([
                ...
                // Page redirect
                return redirect()->route('admin.business.offline.transactions')->with('success', trans('Status Updated Successfully!'));
            } else {
                ...
                if (($plan_features['no_of_businesses'] > $purchased_plan_features['no_of_businesses']) || ($plan_features['no_of_services'] > $purchased_plan_features['no_of_services']) || ($plan_features['no_of_employees'] > $purchased_plan_features['no_of_employees'])) {
                    ...
                    // get business ids
                    $business_ids = Business::where('user_id', $transaction_details->user_id)->where('status', '>=', 0)->pluck('business_id');


                    // deactive businesses
                    Business::where('user_id', $transaction_details->user_id)->update([
                        'status' => 0
                    ]);
